...
class MeaningTutorMaterials extends React.Component {
  ...
  handleParagraphAdd = async () => {
    ...
    await axios.put("/api/meaning/materials/" + this.state.id, { paragraphs });
  ...
  handleParagraphDelete = async (index) => {
    ...
    await axios.put("/api/meaning/materials/" + this.state.id, {
  ...
  handleVideoAdd = async (file) => {
    if (!!file) {
      ...
      await axios.put("/api/meaning/materials/" + id, { videos });
      this.componentDidMount();
    }
  };
